...
data {
  ...
  //real logra[N]; // Rt_a_prior
  //real logrb[N]; // Rt_b_prior
  //real logrc[N]; // Rt_c_prior
...
parameters {
  ...
  //vector<lower=0>[N2] mua;
  //vector<lower=0>[N2] mub;
  //vector<lower=0>[N2] muc;
}
...
transformed parameters {
  ...
  
  {
    ...
    //predictiona[1:N2]= predictiona[1:N2] +  mua[1:N2];
    //predictionb[1:N2]= predictionb[1:N2] +  mub[1:N2];
    //predictionc[1:N2]= predictionc[1:N2] +  muc[1:N2];
    for (i in 2:N) {
      ...
      //predictiona[i] = predictiona[i] + Rta[i] * convolutiona * 0.8 + Rtb[i] * convolutionb * 0.4 ;
      //predictionb[i] = predictionb[i] + Rta[i] * convolutiona * 0.2 + Rtb[i] * convolutionb * 0.6 ;
...
model {
  ...
  //phi ~ normal(0,5);
  //mua[1:N2] ~ exponential(0.5);
  //mub[1:N2] ~ exponential(0.5);
  //muc[1:N2] ~ exponential(0.5);
...
fit <- sampling(stan_model1,data=stan_data,iter=2000,warmup=1000,chains=5,thin=5,control = list(adapt_delta = 0.99, max_treedepth = 30))
