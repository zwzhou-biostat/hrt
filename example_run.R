...
result <- estimate_R_adj_inc(incidence_data, P, si_shape, si_rate, si_t_max, start_day, window_length)
...
## The extracted result 'dat_regions' contains the posterior mean of R(t) as well as 2.5% quantile and 97.5% quantile of the posterior distribution

#### Generate plot for R(t)
dat_plot_r <- dat_regions %>% rename(r = "Mean(R)", rl = "Quantile.0.025(R)", ru = "Quantile.0.975(R)")
...
dat_inc_est_list <- lapply(1:3, function(m){
    dat_regions_R_est <- spread(dat_regions[, c(names(dat_regions)[m], "region", "day")], region, names(dat_regions)[m])[, -1]
    inc_est <- as.data.frame(t(sapply(2:nrow(dat_regions_R_est), function(t){
      rev(w[1:min(length(w), t)])%*%as.matrix(incidence_data[c(max(t-length(w) + 1, 1): t), 1:3,drop = F])%*%diag(dat_regions_R_est[t, ])%*%P
    })))
