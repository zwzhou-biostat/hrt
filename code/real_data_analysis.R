...
avg_mobility_matrix <- readRDS("avg_mobility_matrix_ma_all_counties.rds")

avg_mobility_matrix_list <- readRDS("avg_mobility_matrix_ma_all_counties_list.rds")

p <- dim(avg_mobility_matrix)[1]
...
P_list <- lapply(avg_mobility_matrix_list[1:length(seq(as.Date("2020-07-01"), as.Date("2021-01-28"), by = "day"))], function(x){t(matrix(x, p))})
...
for(i in dir("G:/My Drive/Boston University/Laura/EpiEstim/R/", full.names = T)){
  source(i)
}
...
shrink_scale <- 1
round_num <- 0
while(sum(sapply(order(apply(case_matrix, 2, mean)), function(x){sum(round(case_matrix_nomix[, x] )< 0)}) > 0)){
    ...
    for(d in 1:212){
      ...
      while(case_matrix_nomix[d, k] < 0){
        ...
        for(i in c(1:14)[-k]){
          ...
          P_temp[i, i] <- P_temp[i, i] + P_temp[k, i]*(1-shrink_scale)
          
          P_temp[k, i] <- P_temp[k, i]*shrink_scale
          
          P_temp[k, k] <-  P_temp[k, k] + sum(P_temp[-k, k]*(1-shrink_scale))
          
          P_temp[-k, k] <- P_temp[-k, k]*shrink_scale
...
out <- readRDS(paste0("ma_all_counties_transpose_adjusted_P_out_2000it.rds"))



data_list_i <- lapply(1:p, function(i){
  data.frame(
    x=1:nrow(stan_data$cases),
    y_real=stan_data$cases[, i],
    y = colMeans(out$prediction[, , i]),
    yl = colQuantiles(out$prediction[, , i],probs=c(0.025)),
    yh = colQuantiles(out$prediction[, , i],probs=c(0.975)),
    Rt = colMeans(out$Rt[, , i]),
    Rtl = colQuantiles(out$Rt[, , i],probs=c(0.025)),
    Rth = colQuantiles(out$Rt[, , i],probs=c(0.975)),
    region = names(MA_cases_count_wide_sub)[i+1]
  )
})

data <- do.call("rbind", data_list_i)

data <- as.data.frame(data)

data$x <- MA_cases_count_wide_sub$date[1:nrow(stan_data$cases)] + 7

names(data)[ncol(data)] <- "County"

data$County <- factor(data$County, levels = geoids, labels = county_names)

date_rante <- seq(as.Date("2020-07-08"), as.Date("2021-01-28"), by = "day")
...
inc_scale_plot_data <- inc_scale_plot_data[inc_scale_plot_data$day %in% date_rante, ]

inc_renewal_equ_plot_data <- data[data$x %in% date_rante, ]
...
out <- readRDS(paste0("ma_all_counties_v1_out_2000it_seperate.rds"))
...
inc_scale_plot_data_nomob <- left_join(inc_scale_plot_data_nomob, dat_plot_nomob, by = c("region", "day"))

inc_scale_plot_data_nomob <- na.omit(inc_scale_plot_data_nomob)

inc_scale_plot_data_nomob <- inc_scale_plot_data_nomob[inc_scale_plot_data_nomob$day %in% date_rante, ]

inc_renewal_equ_plot_data_seperate <- data[data$x %in% date_rante, ]
...
inc_plot_both <- ggplot(inc_both_method_plot_data[inc_both_method_plot_data$day %in% date_range, ], aes(x = day)) +
  # geom_col(real_inc_data,mapping=aes(y=y_real,x=day),size=1.2) +
  ...
  # facet_wrap( ~ region, nrow = 2, scales = "fixed")  +
  theme_classic() +  xlab("Days") + ylab('Incidence')+
  scale_x_date(date_breaks = "1 month", date_labels =  "%b %y") + theme(axis.text.x = element_text(angle = 45, vjust = 1.45, hjust=1.3))
...
rt_plot_both <- ggplot(inc_both_method_plot_data[inc_both_method_plot_data$day %in% date_range, ], aes(x = day, color = Method, fill = Method)) +
  ...
  facet_wrap( ~ region, nrow = 2, scales = "free_y")  +
  # facet_wrap( ~ region, nrow = 2, scales = "fixed")  +
  theme_classic() +  xlab("Days") + ylab('Reproductive Number')+
  scale_x_date(date_breaks = "1 month", date_labels =  "%b %y") + theme(axis.text.x = element_text(angle = 45, vjust = 1.45, hjust=1.3)) 
...
county_ids <- data.frame(geoid = ma_fips, 
                         name = county_names)
...
mobility_ma_list_nonames <- lapply(mobility_ma_list, function(x){
  mobility_meta <- data.frame(geoid_o = rep(county_ids$geoid, each = p),
...
date_range <- seq(as.Date("2020-07-20"), as.Date("2021-01-28"), by = "day")
...
g_scale_r <- ggplot(s_p_vec_long_sub, aes(y = pop, x = day)) + geom_line() +  facet_wrap( ~ region, nrow = 3) +geom_hline(yintercept = 0,color = "black", size=1,alpha=0.5) +
  facet_wrap( ~ region, nrow = 2)  +
  theme_classic() +  xlab("Days") + ylab('Population Change')+
  scale_x_date(date_breaks = "1 month", date_labels =  "%b %y") + theme(axis.text.x = element_text(angle = 45, vjust = 1.35, hjust=1.3))
...
max_y_by_county <- real_inc_data %>% group_by(region) %>% summarize(max_y = max(y_real))
county_levels <- max_y_by_county$region[order(max_y_by_county$max_y, decreasing = T)] %>% as.character()

s_p_vec_long_sub$region <- factor(s_p_vec_long_sub$region, levels = county_levels)

g_scale_r <- ggplot(s_p_vec_long_sub[s_p_vec_long_sub$day %in% date_range, ], aes(y = pop, x = day)) + geom_line() +  facet_wrap( ~ region, nrow = 3) +geom_hline(yintercept = 0,color = "black", size=1,alpha=0.5) +
  facet_wrap( ~ region, ncol = 8)  +
  theme_classic() +  xlab("Days") + ylab('Population Change')+
  scale_x_date(date_breaks = "1 month", date_labels =  "%b %y") + theme(axis.text.x = element_text(angle = 60, vjust = 1.35, hjust=1.3))

inc_both_method_plot_data$region <-  factor(inc_both_method_plot_data$region, levels = county_levels)

dummy_data <- data.frame(value = c(rep(c(0, range(real_inc_data$y_real[real_inc_data$region == "Middlesex"])[2] + 1), 8), rep(c(0, range(real_inc_data$y_real[real_inc_data$region == "Barnstable"])[2] + 1), 6)), region = rep(county_levels, each = 2), day = real_inc_data[50, 1])

dummy_data$region <- factor(dummy_data$region, levels = county_levels)

inc_both_method_plot_data <- inc_both_method_plot_data[inc_both_method_plot_data$day %in% date_range, ]


inc_plot_both <- ggplot(inc_both_method_plot_data, aes(x = day)) +
  geom_col(real_inc_data[real_inc_data$day %in% date_range, ],mapping=aes(y=y_real,x=day),size=1.2) +
  geom_line(inc_both_method_plot_data,mapping=aes(y=y,x=day, color = Method),alpha=0.6,size=0.6) +
  # geom_line(data = data, aes(y = y_real, color = County)) +
  geom_ribbon(inc_both_method_plot_data,mapping=aes(x=day,ymin=yl,ymax=yh, color = Method, fill = Method),alpha=0.3, colour = NA) +
  geom_blank(data = dummy_data[dummy_data$day %in%  date_range, ], aes(y = value)) +
  # facet_wrap( ~ region, nrow = 2, scales = "free_y")  +
  facet_wrap( ~ region, ncol = 8, scales = "free_y")  +
  theme_classic() +  xlab("Days") + ylab('Incidence')+
  scale_x_date(date_breaks = "1 month", date_labels =  "%b %y") + theme(axis.text.x = element_text(angle = 60, vjust = 1.35, hjust=1.3))

rt_plot_both <- ggplot(inc_both_method_plot_data, aes(x = day, color = Method, fill = Method)) +
  # geom_col(real_inc_data,mapping=aes(y=y_real,x=day),size=1.2) +
  geom_line(inc_both_method_plot_data,mapping=aes(y=r,x=day, color = Method),alpha=0.6,size=0.6) +
  # geom_line(data = data, aes(y = y_real, color = County)) +
  geom_ribbon(inc_both_method_plot_data,mapping=aes(x=day,ymin=rl,ymax=ru, color = Method),alpha=0.3, colour = NA) +
  # facet_wrap( ~ region, nrow = 2, scales = "free_y")  +
  facet_wrap( ~ region, ncol = 8, scales = "free_y")  +
  theme_classic() +  xlab("Days") + ylab('Reproductive Number')+
  scale_x_date(date_breaks = "1 month", date_labels =  "%b %y") + theme(axis.text.x = element_text(angle = 60, vjust = 1.35, hjust=1.3)) 
...
jpeg("real_data_analaysis.jpg", res = 300, w = 3670, h = 3000)
ggdraw() +
